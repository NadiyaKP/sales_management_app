...
// Data models from receiptreport.dart
  ...
  factory CollectionReport.fromJson(Map<String, dynamic> json) {
    return CollectionReport(
      ...
      customer: json["customer"] != null
          ? CollectionCustomer.fromJson(json["customer"])
          : CollectionCustomer(
...
class ReceiptReportPage extends StatefulWidget {
  const ReceiptReportPage({Key? key}) : super(key: key);
...
class _ReceiptReportPageState extends State<ReceiptReportPage> {
  ...
  List<CollectionReport> allCollectionReports = []; // Store all collection reports
  List<CollectionReport> filteredCollectionReport = []; // Store filtered collection reports
  ...
  void _filterCollectionReports() {
    if (searchQuery.isEmpty) {
      filteredCollectionReport = List.from(allCollectionReports);
    } else {
      filteredCollectionReport = allCollectionReports.where((report) {
  ...
  Future<void> fetchCollection() async {
    ...
    try {
      final response = await http.post(
        Uri.parse('$url/collection-report.php'),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['result'] == "1") {
          final List<dynamic> collectionReport = data['receipts'] ?? [];
          collectionTotal = data['total_amount'];
    ...
    try {
      ...
      if (permissionResponse.permissionDetails.isNotEmpty) {
        setState(() {
          ...
          receiptReport = permissionDetail.receiptReport;
        });
      } else {
  ...
  Future<void> _exportToExcelExternal() async {
    if (filteredCollectionReport.isEmpty) {
    ...
    try {
      ...
      // Add data rows
      for (int i = 0; i < filteredCollectionReport.length; i++) {
        final receipt = filteredCollectionReport[i];
        final rowIndex = i + 2;
        
        sheetObject.cell(CellIndex.indexByString("A$rowIndex")).value = i + 1;
        sheetObject.cell(CellIndex.indexByString("B$rowIndex")).value = receipt.receiptNo;
        sheetObject.cell(CellIndex.indexByString("C$rowIndex")).value = receipt.date;
        sheetObject.cell(CellIndex.indexByString("D$rowIndex")).value = capitalizeWords(receipt.customer.customerName);
      }
  ...
  Future<void> _printReport() async {
    ...
    try {
      ...
      final List<List<String>> tableData = [
        ['SI.No.', 'Receipt No', 'Date', 'Customer Name'], // Header
      ];
      ...
      for (int i = 0; i < filteredCollectionReport.length; i++) {
        ...
        tableData.add([
          ...
          receipt.receiptNo,
          receipt.date,
          capitalizeWords(receipt.customer.customerName),
        ]);
      }
          ...
          build: (pw.Context context) {
            return [
              ...
              pw.Padding(
                ...
                child: pw.Text(
                  'Total Receipts: ${filteredCollectionReport.length}',
                  style: pw.TextStyle(
                    fontSize: 12,
                    fontWeight: pw.FontWeight.bold,
                  ),
      ...
      builder: (_) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: const InputDecoration(
              hintText: 'Enter customer name, receipt no, phone, wallet, or notes'),
        ),
        actions: [
          TextButton(
            onPressed: () {
  ...
  double _calculateTotalReceived() {
    if (collectionTotal == null || collectionTotal!.isEmpty) return 0.0;
    try {
      return double.parse(collectionTotal!);
    } catch (e) {
      return 0.0;
    }
  }
      ...
      child: Column(
        ...
        children: [
          ...
          InkWell(
              ...
              child: Text(
                ...
                overflow: TextOverflow.ellipsis,
                  ...
                  if (searchQuery.isNotEmpty) ...[
                    Text(
                      'Found ${filteredCollectionReport.length} results for "$searchQuery"',
                      style: TextStyle(
                        fontSize: 12,
            ...
            // Receipt List
            Expanded(
              child: filteredCollectionReport.isEmpty
                  ? Center(
                      child: Text(
                  ...
                  : ListView.builder(
                      ...
                      itemCount: filteredCollectionReport.length,
                      itemBuilder: (context, index) {
                        final report = filteredCollectionReport[index];
